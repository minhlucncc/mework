...
> tokens are stored and verified, how provider credentials are sealed, the grant model,
> and the required environment variables. Status badges:
> **`[Implemented]`** today; **`[Planned]`** if not yet built.
>
> Source of truth: `libs/server/{auth,middleware}/`,
> `libs/server/platform/{token,secret}/`, `libs/server/hub/config.go`,
> `libs/shared/config/`.
...
| **`rt_token` / runner identity** `[Implemented]` | the daemon | `/api/v1/jobs/*`, `/api/v1/runners/sessions/*`, agent pull | plaintext once at creation/enroll; only an HMAC hash stored server-side; durable at `~/.mework/identity.json` `0600` |
...
| **registration token** `[Implemented]` | the operator → runner | one-time runner enrollment (`runner enroll`) | short-lived, single-use |
| **grant** `[Implemented]` | travels with each dispatch | the operations a run may perform (`OpPullAgent`, `OpSpawn`) | signed/sealed, per-run |

> `SERVER_KEY` and `MEWORK_SECRET_KEY` are **required** and must be **at least 16
> characters** — the server fails fast at startup otherwise.
...
`libs/server/auth/pat.go`. A bearer **Mello personal access token** authenticates
...
`libs/server/middleware/runtime_auth.go`. A bearer runtime token (`mework_rt_…`)
...
**Token properties** (`libs/server/platform/token/`): a recognizable prefix + 256-bit
...
handler. The Mello adapter (`libs/server/provider/mello/adapter.go`) verifies
...
### Open
`/healthz`, `/livez`, and `/readyz` are unauthenticated.

## Credential sealing `[Implemented]`

`libs/server/platform/secret/secret.go`. Provider tokens (used for REST write-back) are
...
## Runner enrollment `[Implemented]`

The primary enrollment path (the legacy `runtime register` remains for backward
compatibility). Modeled on `actions/runner config`:
...
### Server (`libs/server/hub/config.go`)

| Env | Required | Default | Purpose |
|-----|----------|---------|---------|
| `DATABASE_URL` | **yes** | — | Postgres DSN |
| `SERVER_KEY` | **yes** (≥16 chars) | — | HMAC key for `rt_token` lookup hashing |
| `MEWORK_SECRET_KEY` | **yes** (≥16 chars) | — | AES-256-GCM key for sealing provider credentials |
...
| `CHANNEL_ROUTING_ENABLED` | no | `false` | Opt-in to experimental channel auto-provisioning (off by default) |
| `STORAGE_*` | no | `fs` | Object storage backend (`STORAGE_DRIVER` = `fs`/`s3`/`minio`/`r2`, + endpoint/bucket/creds) |
...
This applies to `~/.mework/config.json`, `daemon.pid`, `daemon.log`, the runner identity
(`~/.mework/identity.json`), and per-job work directories.
