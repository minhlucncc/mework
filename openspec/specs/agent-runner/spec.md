...
Define the agent runner: a lightweight daemon that is installed once on a
developer's machine, enrolled against the hub, and thereafter receives and
executes dispatched agents over SSE. The runner pulls agent versions from the
catalog, runs them in a sandbox, enforces per-dispatch permission grants, and
reports results. Owned by the runner codebase.
