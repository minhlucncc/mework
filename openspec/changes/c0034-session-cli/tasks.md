...
- [ ] 1.1 Write `cmd_session_test.go` (fail first): `httptest` stub for `GET
      /api/v1/sessions` returning sessions; assert the table render and `--json` output;
      missing PAT → guidance error.
- [ ] 1.2 Replace the stub `sessionListCmd` body with a real `GET /api/v1/sessions` (PAT
      auth), rendering the existing table header + rows.

## 2. `session create` (TDD)

- [ ] 2.1 Write a test (fail first): `create --agent X [--runner R] [--version V] [--json]`
      POSTs `/api/v1/sessions` with the right body and prints the returned session id.
- [ ] 2.2 Implement `sessionCreateCmd`.

## 3. `session send` (TDD)

- [ ] 3.1 Write a test (fail first): `send <id> <msg>` POSTs
      `/api/v1/sessions/{id}/messages` with `{role:"user", content}` and treats 202 as
      success.
- [ ] 3.2 Implement `sessionSendCmd`.

## 4. `session attach` (TDD)

- [ ] 4.1 Write a test (fail first): stub an SSE response with two `data:` `ChatEvent`
...
- [ ] 4.2 Implement `sessionAttachCmd`: open `GET /api/v1/sessions/{id}/stream`, decode
      frames, print `token`/`message` content, stop on `done`/`error` or idle timeout.

## 5. `session close` + registration

- [ ] 5.1 Implement `sessionCloseCmd` (`DELETE /api/v1/sessions/{id}`).
- [ ] 5.2 Register `create`/`send`/`attach`/`close` under `sessionCmd` in `init()`.

## 6. Validation

- [ ] 6.1 `make vet` and `make test ./libs/client/cli/...` green; new tests fail-first then
      pass.
