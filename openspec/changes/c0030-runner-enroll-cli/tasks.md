...
- [ ] 1.1 Write `libs/client/cli/cmd_runner_test.go` (fail first): table test with an
...
- [ ] 1.2 Replace the stub body of `runnerEnrollCmd.RunE` (`libs/client/cli/cmd_runner.go`):
...
- [ ] 1.3 Remove now-dead helpers (e.g. `quickLen`) if no longer referenced.

## 2. Validation

- [ ] 2.1 `make vet` and `make test ./libs/client/cli/...` green; new test fails before the
